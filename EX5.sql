...
SELECT 
	...
	ROUND((salary-LEAD(salary) OVER(ORDER BY salary DESC))*100/salary) AS diff_percent
FROM staff;
...
SELECT 
	...
	LAG(grade, 1, 'нет оценки') OVER w AS prev_grade, -- смещение на 1 и вместо NULL будет значение 'нет оценки'
	LEAD(grade) OVER w AS last_grade,
	LEAD(grade, 1, 'нет оценки') OVER w AS last_grade -- смещение на 1 и вместо NULL будет значение 'нет оценки'
FROM academic_record
...
SELECT friend_id FROM tmp_friends
WHERE user_id=1;
